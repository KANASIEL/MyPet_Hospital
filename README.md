...
- **증상**: 동일 파일을 여러 명이 수정하여 Merge Conflict 발생

### 🔍 원인 분석
- 서로 다른 브랜치에서 동일한 파일의 동일 라인을 수정
- 커밋 전 최신 `develop` 브랜치를 pull 하지 않은 상태로 작업 진행
- 팀 내 작업 범위 공유 부족

### 🛠 해결 방법
- 충돌 발생 브랜치에서 최신 브랜치 반영
  ```bash
  git pull origin develop
  ```
  또한 동일한 이슈 발생 방지를 위해 협업관리 도구를 사용해 작업을 지속적으로 공유, 백업
